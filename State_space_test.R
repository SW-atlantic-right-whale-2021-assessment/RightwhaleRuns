...
while (i < n_resamples) {
  ...
  if (priors$q_count$use) {
    ...
    check <- data.frame(Year = 1:bisection.Yrs, Proc = sample.proc.error[1:bisection.Yrs], Nhat = Pred_N$Pred_N)
    plot(check$Year, check$Nhat, type = "l")
    
    plot(curve((1-(1.001460e+05 /x)^sample.z), from = 1e5, to = 1e6), type = "l")
